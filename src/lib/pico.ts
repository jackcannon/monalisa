	...
	function classify_region(r, c, s, pixels, ldim)
	{
		 ...
		 for(let i = 0; i < ntrees; ++i)
		 {
			...
			for(let j=0; j<tdepth; ++j) {
				...
				let part1 = 2 * idx;
				let part2 = pixels[((r+tcodes[root + 4*idx + 0]*s) >> 8)*ldim+((c+tcodes[root + 4*idx + 1]*s) >> 8)];
				let part3 = pixels[((r+tcodes[root + 4*idx + 2]*s) >> 8)*ldim+((c+tcodes[root + 4*idx + 3]*s) >> 8)];
				idx = part1 + Number(part2 <= part3);
			}
